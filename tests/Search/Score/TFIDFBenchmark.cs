...
public class TFIDFBenchmark
{
    ...
    public void Setup()
    {
        ...
        _tfidf = new TFIDF(mockIndex.Object);
    }

    [Benchmark]
    public Dictionary<int, double> TFIDF_Calculate_Multiple()
    {
        return _tfidf.Calculate(["search", "engine"]);
    }
    
    [Benchmark]
    public Dictionary<int, double>? TFIDF_Calculate_Single()
    {
        return _tfidf.Calculate(["search"]);
    }
}
